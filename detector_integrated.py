...
# Función para clasificar vehículos según su tamaño
def clasificar_vehiculo(w, h):
    area = w * h
    if area < 55000:
        return "Automovil"
    elif area < 130000:
    ...
    if not video_paused or not zoom_enabled:
        ...
        for count in contours:
            ...
            if area > 800:
                ...
                print(f'Dimenciones del vehiculo detectado - Ancho: {width}, Alto: {height}, Area: {width * height}')
                vehicle_type = clasificar_vehiculo(width, height)  # Clasificar el vehículo
                cv2.rectangle(zone, (x, y), (x + width, y + height), (0, 255, 0), 2)
                cv2.putText(zone, vehicle_type, (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
